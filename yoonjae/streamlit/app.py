    ...
    if st.button('Predict'):
        if selected_start_date > selected_end_date:
            ...
            with st.spinner('Predicting sales... It will take about 2 minutes...'):
                ...
                try:
                    deepAR_origin, feature_importances, df, forecast_sales, forecast_transactions, data = get_predict(sales_data)
                
                    st.write("### Predicted Sales")
                    fomatted= round(deepAR_origin['predicted_sales'],2)
                    st.success(f'The predicted sales is # {fomatted}')
                    st.dataframe(deepAR_origin)  # 상호작용 가능한 테이블                    
                    
                    # deepAR_origin 데이터프레임에 sales 컬럼을 추가
                    deepAR_origin['sales'] = deepAR_origin['predicted_sales']
                    
                    # 자르기
                    deepAR_origin = deepAR_origin[deepAR_origin['date'] >= '2017-06-15']
                    data = data[data['date'] >= '2017-06-15']

                    # 필터링된 deepAR_origin 데이터프레임 결합
                    filtered_deepAR_origin = deepAR_origin[(deepAR_origin['store_nbr'] == int(selected_store_nbr)) & (deepAR_origin['family'] == selected_family)]
                    PredictedPlot = pd.concat([
                        filtered_deepAR_origin[['date', 'predicted_sales']], 
                        data[['date', 'sales']]
                    ], axis=0)

                    PredictedPlot.sort_values(by='date', inplace=True)

                    # 그래프 그리기
                    plt.figure(figsize=(10, 3))
                    plt.plot(PredictedPlot['date'], PredictedPlot['sales'], color="black", label="True values")
                    plt.plot(PredictedPlot['date'], PredictedPlot['predicted_sales'], color="blue", label="Predicted")
                    plt.legend(loc="upper left")
                    plt.title('RandomForestRegressor Predicted Sales')
                    st.pyplot(plt)
                        
                    # Plot predictions
                    plt.figure(figsize=(10, 3))
                    plt.plot(df.index[-60:], df['sales'][-60:], color="black", label="True values")
                    for forecast in forecast_sales:
                        plt.plot(forecast.index, forecast.mean, label="Forecast")
                    plt.legend(loc="upper left")
                    plt.title('gluonTS DeepAR Forecast sales')
                    plt.show()
                    st.pyplot(plt)
                    ...
                    for forecast in forecast_transactions:
                        plt.plot(forecast.index, forecast.mean, label="Forecast")
                    plt.legend(loc="upper left")
                    plt.title('gluonTS DeepAR Forecast transactions')
                    plt.show()
                    st.pyplot(plt)
                    
                    st.write("### Feature Importances")
                    # Input form
                    #col1_feature, co2_feature = st.columns(2)
                    #with col1_feature:
...
###################################################### section : EDA ######################################################
    ...
    if st.button('EDA start'):
        
        st.write('### Explore train.csv')
        # 디버깅 용도
        ...
        st.write('### Missing Values: transactions & sales')
        ...
        st.write('### Average Sales by Day_of_week')
        ...
        st.write('### Corr : Number of Promotion-items & Sales')
        ...
        st.write('### outlier detection with onpormotion')
        ...
        st.write('### feature selection: corr heatmap')
        temp = pd.read_csv(r'C:\Users\user\upstage3-python\09.ML_project_team03\assets\data\temp2.csv')
        temp['date'] = pd.to_datetime(temp['date'])
